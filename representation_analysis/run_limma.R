...
adata$obs <- adata$obs %>%
  mutate(sm_cell_type_donor = paste(sm_cell_type, donor_id, sep = "_"))

# select [cell_type, sm_name] pairs which will be used for DE analysis
new_obs <- adata$obs %>%
  select(cell_type, sm_name, sm_lincs_id, SMILES, split, control, sm_cell_type_donor, donor_id) %>%
  distinct() %>%
...
design_matrix <- model.matrix(~ 0 + sm_cell_type_donor, adata[obs_filt, ]$obs %>% mutate_all(limma_trafo))
...
de_df <- future_map_dfr(
  ...
  function(row_i) {
    ...
    sm_cell_type_donor <- as.character(new_obs$sm_cell_type_donor[[row_i]])
    cell_type <- as.character(new_obs$cell_type[[row_i]])
    donor_id <- as.character(new_obs$donor_id[[row_i]])

    control_name <- paste(par$control_compound, cell_type, sep = "_")
    control_name <- paste(control_name, donor_id, sep = "_")
    # run contrast fit
    contrast_formula <- paste0(
      "sm_cell_type_donor", limma_trafo(sm_cell_type_donor),
      " - ",
      "sm_cell_type_donor", limma_trafo(control_name)
    )
...
rownames(new_obs) <- paste0(new_obs$cell_type, ", ", new_obs$sm_name, ", ", new_obs$donor_id)
